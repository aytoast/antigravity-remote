...
router.get('/codex/threads/:id/model', (req, res) => {
    try { res.json({ success: true, data: { model: codexBridge.getDesktopThreadModel(req.params.id) } }); }
    catch (error) { res.status(503).json({ success: false, error: error.message }); }
});
...
router.get('/codex/desktop/activity', (req, res) => {
    try { res.json({ success: true, data: { active: codexBridge.getDesktopTurnActive() } }); }
    catch (error) { res.status(503).json({ success: false, error: error.message }); }
});

router.post('/codex/desktop/threads/:id/prompt', async (req, res) => {
    if (typeof req.body?.prompt !== 'string' || !req.body.prompt.trim()) return res.status(400).json({ success: false, error: 'prompt is required' });
    try {
        await codexBridge.openDesktopThread(req.params.id);
...
router.post('/codex/desktop/threads/:id/open', async (req, res) => {
    try { res.json({ success: true, data: await codexBridge.openDesktopThread(req.params.id) }); }
    catch (error) { res.status(503).json({ success: false, error: error.message }); }
});
...
router.put('/codex/threads/:id/pin', async (req, res) => {
    if (typeof req.body?.pinned !== 'boolean') return res.status(400).json({ success: false, error: 'pinned is required' });
    try { res.json({ success: true, data: await codexBridge.setThreadPinned(req.params.id, req.body.pinned) }); }
    catch (error) { res.status(503).json({ success: false, error: error.message }); }
});
